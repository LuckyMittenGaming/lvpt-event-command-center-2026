...
async function dataUrlFile(item){const blob=await fetch(item.dataUrl).then(r=>r.blob());return new File([blob],item.name,{type:blob.type||'image/jpeg'})}
async function nativeMultipart(data,images){
  const token='lvpt_'+Date.now()+'_'+Math.random().toString(36).slice(2),frame=document.createElement('iframe');
  frame.name=token;frame.style.cssText='position:fixed;width:1px;height:1px;opacity:0;pointer-events:none;left:-9999px;top:-9999px;border:0';
  document.body.appendChild(frame);
  const form=document.createElement('form');form.method='POST';form.action='https://formsubmit.co/'+notify;form.enctype='multipart/form-data';form.target=token;form.style.display='none';
  const s=data.selections,b=data.beverage;
  const successUrl=location.origin+'/submission-received.html?lvpt=1&t='+encodeURIComponent(token);
  const fields={_subject:client+' submitted LVPT event selections',_template:'table',_captcha:'false',_cc:data.email,_replyto:data.email,_next:successUrl,_url:location.origin+'/',Client:client,Event:eventName,Event_Date:eventDate||'Not provided',Event_Time:eventTime||'Not provided',Expected_Guests:expectedGuests||'Not provided',Submitted_By:data.submittedBy,email:data.email,Chip_Selection:s.chip,Card_Selection:s.card,Culinary_Selection:s.culinary,Bar_Experience:s.bar,Beer_Choice_1:b.beer1||'None',Beer_Choice_2:b.beer2||'None',Red_Wine_1:b.red1||'None',Red_Wine_2:b.red2||'None',White_Wine_1:b.white1||'None',White_Wine_2:b.white2||'None',Premium_Liquor_1:b.liq1||'None',Premium_Liquor_2:b.liq2||'None',Premium_Liquor_3:b.liq3||'None',Comments:data.comments,Submitted_At:new Date().toLocaleString(),Portal_URL:location.href};
  Object.entries(fields).forEach(([k,v])=>addHidden(form,k,v));
  for(let n=0;n<images.length;n++){const file=await dataUrlFile(images[n]),input=document.createElement('input'),dt=new DataTransfer();input.type='file';input.name=n?'attachment_'+(n+1):'attachment';dt.items.add(file);input.files=dt.files;form.appendChild(input)}
  document.body.appendChild(form);
  return await new Promise((resolve,reject)=>{let done=false,submitted=false;const clean=()=>{setTimeout(()=>{form.remove();frame.remove()},100)};const timer=setTimeout(()=>{if(done)return;done=true;clean();reject(new Error('Attachment submission did not confirm'))},20000);frame.addEventListener('load',()=>{if(!submitted||done)return;try{const href=frame.contentWindow.location.href;if(href.startsWith(location.origin+'/submission-received.html')){done=true;clearTimeout(timer);clean();resolve({attached:images.length})}}catch(e){/* still on FormSubmit; wait for _next redirect */}});submitted=true;form.submit()})
}
...
async function richSubmit(ev){const btn=ev.target.closest?.('#submit');if(!btn)return;ev.preventDefault();ev.stopImmediatePropagation();const status=q('#status'),name=val('who'),email=val('whoEmail');if(!name||!email){status.textContent='Please add your name and email.';return}if(!q('#confirm')?.checked){status.textContent='Please check the confirmation box before submitting.';return}btn.disabled=true;status.textContent='Preparing your selected designs…';const data=payload(name,email);try{const images=await imagePayload();status.textContent='Sending selections, images and confirmation copy…';const d=await nativeMultipart(data,images);status.innerHTML='<div class="success"><strong>Selections received.</strong><br>Your choices and '+(d.attached||0)+' selected image'+((d.attached||0)===1?'':'s')+' were sent to Las Vegas Poker Training. A confirmation copy was sent to <strong>'+email.replace(/[<>&]/g,'')+'</strong>.</div>';btn.textContent='Selections Submitted'}catch(err){console.error('Attachment submission failed; using reliable fallback',err);try{status.textContent='Sending your selections…';await fallback(data);status.innerHTML='<div class="success"><strong>Selections received.</strong><br>Your event choices were delivered successfully. The image attachment submission could not be confirmed, so the text selections were also sent as a fallback.</div>';btn.textContent='Selections Submitted'}catch(err2){console.error(err2);status.textContent='We could not send automatically. Please try again or contact Las Vegas Poker Training directly.';btn.disabled=false}}}
