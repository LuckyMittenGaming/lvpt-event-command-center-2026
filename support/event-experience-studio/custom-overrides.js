...
function imageData(file,max=1500){
  return new Promise((res,rej)=>{
    ...
    r.onload=()=>{
      ...
      im.onload=()=>{
        ...
        try{res(c.toDataURL('image/webp',.84))}catch{res(r.result)}
      };
      im.src=r.result;
    };
...
.culinaryAdminCard .hero{background:linear-gradient(145deg,#151515,#090909);display:flex;align-items:center;justify-content:center;overflow:hidden;min-height:180px}
...
function renderAdmin(){
  ...
  if(fs){
    ...
    f.querySelectorAll('[data-cul-img]').forEach(x=>x.onchange=async()=>{try{const a=rd(),q=a.find(z=>z.id===aid());ensure(q);q.culinaryOptions[+x.dataset.culImg].image=await imageData(x.files?.[0],1500);if(!save(q))alert('That image could not be saved because browser storage is full. Try a smaller file or remove unused proof artwork.');renderAdmin()}catch(err){console.error(err)}});
  }
  ...
  if(bs){
    ...
    b.querySelectorAll('[data-bev-img]').forEach(x=>x.onchange=async()=>{try{const a=rd(),q=a.find(z=>z.id===aid());ensure(q);q.beverageOptions[+x.dataset.bevImg].image=await imageData(x.files?.[0],1500);if(!save(q))alert('That image could not be saved because browser storage is full. Try a smaller file or remove unused proof artwork.');renderAdmin()}catch(err){console.error(err)}});
  }
  lock=false;
}
